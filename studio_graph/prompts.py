...
{{
  "characters": {{
    "char_anna": {{
      ...
      "base_prompt_en": "young adult female, straight black hair, brown eyes, wearing a white shirt and red apron"
    }}
  }},
  "panels": [
    {{
...
You MUST design the characters in the `"characters"` block first. The `base_prompt_en` MUST be highly specific about clothing color and type (e.g. "wearing a white shirt and red apron"). DO NOT use vague terms like "casual clothes".
The Renderer will lock the appearance based on `base_prompt_en`. Ensure you use the exact `char_id` (e.g. "char_anna") in `active_char_ids` for EVERY panel they appear in!
...
Check for:
1. Valid JSON format and structure. MUST include a `"characters"` dictionary and `"panels"` array.
2. Prompts are not too long (CLIP token limits ~75 tokens).
3. **CHARACTER CONSISTENCY (CRITICAL)**: 
   - Ensure the `"characters"` block exists.
   - The `base_prompt_en` of every character MUST explicitly mention clothing color and type (e.g. "red shirt", "blue apron"). If it just says "casual clothes" or is missing clothing details, REJECT it.
